...
## Data Quality — 8 Injected & Fixed

| # | Issue | Fix |
|---|-------|-----|
| EDA | Unsalvageable rows (4+ nulls) | dropna(thresh=4) — drop first, before any cleaning |
| 1 | Mixed date formats (dash / slash / written) | pd.to_datetime(dayfirst=True) — handles all 3 formats in one shot |
| 2 | Inconsistent string formatting (city case, payment_method) | str.title() standardization |
| 3 | Null values (email, phone, qty) | fillna('Unknown') / smart imputation — null qty derived from total ÷ unit_price |
| 4 | Invalid values (negative age, zero qty) | Negative age → mean of valid ages / zero qty → derived from total ÷ price |
| 5 | Orphan records (orders with non-existent customer_id) | Cascade delete orders → order_items (referential integrity enforced manually) |
| 6 | Outliers (unit_price, order_amount) | IQR subcategory-wise — not global; a ₹500 phone is an outlier, a ₹500 t-shirt is not |
| 7 | Wrong data types (dates as strings, numeric as object) | astype() + pd.to_datetime() + pd.to_numeric() |
| 8 | Duplicate rows + duplicate primary keys | drop_duplicates() on rows → primary key check → remove imputation-generated duplicates |
